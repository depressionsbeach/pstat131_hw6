...
```
< br />
**It performs better with a smaller complexity penalty.** 
...
```{r}
pokemon_final <- tune_res %>%
  select_best(metric = 'roc_auc') %>%
  finalize_workflow(x = pokemon_wf) 

pokemon_fit_folds <- pokemon_final %>%
  fit_resamples(resamples = pokemon_folds)

collect_metrics(pokemon_fit_folds)[2, c(1,3,5)]
```
